...
function PatchLinha({ patch, onEditarNome, onApagar, onUploadFoto }) {
  ...
  return (
    <span className="patch">
      <button type="button" className="patch-foto" onClick={() => input.current?.click()} title={fotoUrl ? t("patch.trocarFoto") : t("patch.foto")}>
        {fotoUrl
      ...
      <input
        className="patch-nome" placeholder={t("patch.ph")} value={patch.nome}
        data-patch-id={patch.id}
        onChange={(e) => onEditarNome(patch.id, e.target.value)}
      />
      <button type="button" className="patch-x" title={t("patch.remover")} onClick={() => onApagar(patch)}>✕</button>
