...
function migrar(db) {
  db.exec(`
    ...
    -- Catálogo de produtos (modelo base de camisa). Cada produto gera variantes
    -- automáticas por tamanho × versão (ex.: Adepto/Jogador), cada uma com SKU.
    CREATE TABLE IF NOT EXISTS produtos (
      id         TEXT PRIMARY KEY,
      nome       TEXT NOT NULL DEFAULT '',
      categoria  TEXT NOT NULL DEFAULT '',
      versoes    TEXT NOT NULL DEFAULT '',  -- JSON: ["Adepto","Jogador"]
  ...
  // fluxo de caixa: na Vinted o dinheiro fica pendente até o comprador receber.
  // data_recebido vazio = venda pendente; com data = recebida/concluída (e só aí
  // se pode acertar com o sócio). Vendas novas nascem pendentes.
  garantirColuna(db, "itens", "data_recebido", "TEXT NOT NULL DEFAULT ''");
  // desconto do vendedor na compra: aplicado ao custo de cada item do pedido.
  // desconto_tipo = 'pct' (percentagem) ou 'fixo' (€ repartido pelos itens).
  garantirColuna(db, "pedidos", "desconto", "REAL NOT NULL DEFAULT 0");
  garantirColuna(db, "pedidos", "desconto_tipo", "TEXT NOT NULL DEFAULT 'pct'");
  // catálogo: um item pode estar ligado a uma variante de produto (modelo +
  // tamanho + versão), guardando também o SKU para mostrar/procurar.
  garantirColuna(db, "itens", "variante_id", "TEXT");
  garantirColuna(db, "itens", "sku", "TEXT NOT NULL DEFAULT ''");
  ...
  // Migração única: o histórico de vendas já recebeu o dinheiro há muito, por
  // isso marca tudo o que já está vendido como recebido (data_recebido = data da
  // venda). Corre uma só vez, guardada por uma flag em config.
  if (!db.prepare("SELECT 1 FROM config WHERE chave = 'migrou_recebido'").get()) {
    db.exec(
      "UPDATE itens SET data_recebido = data_venda WHERE preco_venda > 0 AND data_venda <> '' AND data_recebido = ''"
    );
    db.prepare("INSERT OR IGNORE INTO config (chave, valor) VALUES ('migrou_recebido', '1')").run();
  }
}
