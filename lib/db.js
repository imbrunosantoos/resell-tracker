...
function migrar(db) {
  db.exec(`
    ...
    -- Acertos de contas com sócios: pagamentos entre ti e um sócio. 'sentido'
    -- diz quem pagou (para_socio = pagaste-lhe a parte dele; do_socio = recebeste).
    CREATE TABLE IF NOT EXISTS acertos (
      id        TEXT PRIMARY KEY,
      socio_id  TEXT REFERENCES socios(id) ON DELETE CASCADE,
      valor     REAL NOT NULL DEFAULT 0,
      sentido   TEXT NOT NULL DEFAULT 'para_socio',
      data      TEXT NOT NULL DEFAULT '',
      nota      TEXT NOT NULL DEFAULT '',
      criado_em TEXT NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_acertos_socio ON acertos(socio_id);
