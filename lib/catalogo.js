...
export async function gerarPdf(linhas, { titulo = "Encomenda", labels = {} } = {}) {
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    // frente + verso, grandes, lado a lado
    const top = M + 22;
    const boxW = (PW - 2 * M - GAP) / 2;
    const boxH = boxW * 1.25;
    await colocarImagem(doc, l.foto, M, top, boxW, boxH, frente);
    await colocarImagem(doc, l.fotoVerso, M + boxW + GAP, top, boxW, boxH, verso);
    doc.setFontSize(10); doc.setTextColor(110);
    doc.text(frente, M, top + boxH + 5);
    doc.text(verso, M + boxW + GAP, top + boxH + 5);
    doc.setTextColor(0);
