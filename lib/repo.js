...
export function lerEstado() {
  ...
  return {
    ...
    rascunho: rascunho.map(linhaParaApi),
    config,
  };
}
...
export function finalizarRascunho(dadosPedido = {}) {
  ...
  try {
    ...
    for (const linha of linhas) {
      ...
      if (linha.foto) apagarFoto(linha.foto); // a foto original da linha já não é precisa
    }
    db.exec("DELETE FROM rascunho");
    db.exec("COMMIT");
    // devolve o pedido já com os itens
    const itens = db.prepare("SELECT * FROM itens WHERE pedido_id = ? ORDER BY ordem ASC").all(pedido.id);
    return { ...pedido, itens: itens.map(itemParaApi) };
  } catch (erro) {
    db.exec("ROLLBACK");
    throw erro;
  }
