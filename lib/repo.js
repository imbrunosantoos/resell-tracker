...
const despesaParaApi = (r) => ({ id: r.id, nome: r.nome, valor: r.valor, periodo: r.periodo });
...
export function lerEstado() {
  ...
  const credenciais = db.prepare("SELECT * FROM credenciais ORDER BY plataforma ASC").all();
  ...
  return {
    ...
    credenciais: credenciais.map(credencialParaApi),
    config,
  };
}
...
export function criarItem(pedidoId, dados = {}) {
  ...
  getDB()
    .prepare(
      `INSERT INTO itens (id, pedido_id, nome, categoria, preco_compra, preco_venda, data_venda, foto, notas, ordem)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id, pedidoId,
