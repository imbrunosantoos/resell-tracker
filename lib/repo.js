...
const produtoParaApi = (r) => ({
  id: r.id, nome: r.nome, categoria: r.categoria, skuBase: r.sku_base || "",
  versoes: parseJson(r.versoes, []),
});
const varianteParaApi = (r) => ({
  id: r.id, produtoId: r.produto_id, tamanho: r.tamanho || "", versao: r.versao || "", sku: r.sku || "",
});
...
// Repartição de tamanhos guardada como JSON (tamanho→qtd). Lê sempre para objeto.
const parseTamanhos = (s) => {
...
const linhaParaApi = (r) => ({
  ...
  fotoVerso: r.foto_verso || "", tamanhos: parseTamanhos(r.tamanhos),
});
...
const CAMPOS_PEDIDO = {
  ...
  desconto: { col: "desconto", conv: Number },
  descontoTipo: { col: "desconto_tipo", conv: (v) => (v === "fixo" ? "fixo" : "pct") },
};
...
const CAMPOS_ITEM = {
  ...
  dataRecebido: { col: "data_recebido", conv: String },
...
const CAMPOS_LINHA = {
  ...
  precoCompra: { col: "preco_compra", conv: Number },
  foto: { col: "foto", conv: String },
  fotoVerso: { col: "foto_verso", conv: String },
  tamanhos: { col: "tamanhos", conv: (v) => (typeof v === "string" ? v : JSON.stringify(v || {})) },
};
...
export function lerEstado() {
  ...
  // catálogo: produtos com as suas variantes (tamanho × versão)
  const produtos = db.prepare("SELECT * FROM produtos ORDER BY criado_em DESC").all();
  const variantes = db.prepare("SELECT * FROM variantes ORDER BY rowid ASC").all();
  const porProduto = new Map(produtos.map((p) => [p.id, []]));
  for (const v of variantes) porProduto.get(v.produto_id)?.push(varianteParaApi(v));
  ...
  return {
    ...
    produtos: produtos.map((p) => ({ ...produtoParaApi(p), variantes: porProduto.get(p.id) ?? [] })),
    config,
  };
}
...
export function criarPedido(dados = {}) {
  ...
  getDB()
    .prepare(
      `INSERT INTO pedidos (id, nome, data_compra, data_chegada, taxa_paypal, saco, socio_id, tipo, cliente, data_pagamento, desconto, desconto_tipo, criado_em)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id,
      ...
      num(dados.desconto),
      dados.descontoTipo === "fixo" ? "fixo" : "pct",
...
export function criarEncomenda(dados = {}) {
  ...
  try {
    ...
    const item = criarItem(pedido.id, {
      ...
      dataVenda: dataPag,
      dataRecebido: dataPag, // encomenda é paga adiantado → já concluída
    });
    db.exec("COMMIT");
...
export function criarItem(pedidoId, dados = {}) {
  ...
  getDB()
    .prepare(
      `INSERT INTO itens (id, pedido_id, nome, categoria, preco_compra, preco_venda, data_venda, data_recebido, foto, notas, tamanho, ordem)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id, pedidoId,
...
// Código-base do SKU a partir do nome (sem acentos/espaços, MAIÚSCULAS).
function slugBase(nome) {
  const s = String(nome ?? "").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-zA-Z0-9]/g, "").toUpperCase();
  return s.slice(0, 12) || "SKU";
}
const codVersao = (v) => String(v ?? "").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-zA-Z0-9]/g, "").toUpperCase().slice(0, 3);

// Cria um produto base e gera as variantes (cada tamanho × cada versão), com um
// SKU único por variante. Tamanhos vêm da categoria (tamanhosPara); se a
// categoria não tem tamanhos, fica uma variante por versão (tamanho vazio).
export function criarProduto(dados = {}) {
  const db = getDB();
  const id = randomUUID();
  const nome = String(dados.nome ?? "").trim() || "Produto sem nome";
  const categoria = String(dados.categoria ?? "").trim();
  const versoes = Array.isArray(dados.versoes) ? dados.versoes.map((v) => String(v).trim()).filter(Boolean) : [];
  const skuBase = slugBase(nome);
  const agora = new Date().toISOString();

  db.prepare(
    "INSERT INTO produtos (id, nome, categoria, versoes, sku_base, criado_em) VALUES (?, ?, ?, ?, ?, ?)"
  ).run(id, nome, categoria, JSON.stringify(versoes), skuBase, agora);

  const tamanhos = tamanhosPara(categoria);
  const tams = tamanhos.length ? tamanhos : [""];
  const vers = versoes.length ? versoes : [""];
  const ins = db.prepare("INSERT INTO variantes (id, produto_id, tamanho, versao, sku, criado_em) VALUES (?, ?, ?, ?, ?, ?)");
  for (const tam of tams) {
    for (const ver of vers) {
      const sku = [skuBase, tam, codVersao(ver)].filter(Boolean).join("-");
      ins.run(randomUUID(), id, tam, ver, sku, agora);
    }
  }
  return lerProduto(id);
}
...
function lerProduto(id) {
  ...
  if (!p) return null;
  const variantes = db.prepare("SELECT * FROM variantes WHERE produto_id = ? ORDER BY rowid ASC").all(id);
  return { ...produtoParaApi(p), variantes: variantes.map(varianteParaApi) };
}

export function apagarProduto(id) {
  const db = getDB();
  // desliga os itens que apontavam para variantes deste produto (sem cascade)
  db.prepare("UPDATE itens SET variante_id = NULL, sku = '' WHERE variante_id IN (SELECT id FROM variantes WHERE produto_id = ?)").run(id);
  db.prepare("DELETE FROM variantes WHERE produto_id = ?").run(id);
  db.prepare("DELETE FROM produtos WHERE id = ?").run(id);
}

// Liga (ou desliga, varianteId vazio) um item a uma variante. Ao ligar, copia o
// nome/categoria/tamanho do modelo e grava o SKU no item.
export function atribuirVariante(itemId, varianteId) {
  const db = getDB();
  if (!varianteId) {
    db.prepare("UPDATE itens SET variante_id = NULL, sku = '' WHERE id = ?").run(itemId);
    return itemParaApi(db.prepare("SELECT * FROM itens WHERE id = ?").get(itemId));
  }
  const v = db.prepare("SELECT * FROM variantes WHERE id = ?").get(varianteId);
  if (!v) throw new Error("Variante não encontrada.");
  const p = db.prepare("SELECT * FROM produtos WHERE id = ?").get(v.produto_id);
  db.prepare(
    "UPDATE itens SET variante_id = ?, sku = ?, nome = ?, categoria = ?, tamanho = ? WHERE id = ?"
  ).run(varianteId, v.sku || "", p?.nome ?? "", p?.categoria ?? "", v.tamanho || "", itemId);
  return itemParaApi(db.prepare("SELECT * FROM itens WHERE id = ?").get(itemId));
}

// Atualiza vários itens de uma vez com os MESMOS campos (seleção em massa).
// Aceita qualquer campo da lista branca CAMPOS_ITEM (categoria, tamanho, preço…).
export function atualizarItensEmMassa(ids, parcial = {}) {
  if (!Array.isArray(ids) || ids.length === 0) return 0;
  const sets = [];
  const valores = [];
  for (const [campo, valor] of Object.entries(parcial)) {
    const def = CAMPOS_ITEM[campo];
    if (!def) continue; // ignora campos fora da lista branca
    sets.push(`${def.col} = ?`);
    valores.push(def.conv === Number ? num(valor) : def.conv(valor));
  }
  if (sets.length === 0) return 0;
  const stmt = getDB().prepare(`UPDATE itens SET ${sets.join(", ")} WHERE id = ?`);
  let n = 0;
  for (const id of ids) n += stmt.run(...valores, id).changes;
  return n;
}
...
export function criarLinha(dados = {}) {
  ...
  getDB()
    .prepare(
      `INSERT INTO rascunho (id, nome, categoria, tamanho, quantidade, preco_compra, foto, foto_verso, tamanhos, ordem, criado_em)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id,
      ...
      num(dados.precoCompra),
      String(dados.foto ?? ""),
      String(dados.fotoVerso ?? ""),
      typeof dados.tamanhos === "string" ? dados.tamanhos : JSON.stringify(dados.tamanhos || {}),
      ordem,
      new Date().toISOString()
    );
...
export function apagarLinha(id) {
  const linha = getDB().prepare("SELECT foto, foto_verso FROM rascunho WHERE id = ?").get(id);
  if (linha?.foto) apagarFoto(linha.foto);
  if (linha?.foto_verso) apagarFoto(linha.foto_verso);
...
export function finalizarRascunho(dadosPedido = {}) {
  ...
  try {
    ...
    for (const linha of linhas) {
      ...
      // Expande por tamanho: se houver repartição {tam:qtd}, cria essas quantidades
      // por tamanho; senão, `quantidade` itens com o tamanho único (categorias sem
      // tamanhos). Cada item leva SÓ a foto da frente; o preço de compra fica a 0.
      const mapa = parseTamanhos(linha.tamanhos);
      const entradas = Object.entries(mapa).filter(([, n]) => num(n) > 0);
      const expandido = entradas.length
        ? entradas.flatMap(([tam, n]) => Array.from({ length: Math.round(num(n)) }, () => tam))
        : Array.from({ length: Math.max(1, linha.quantidade || 1) }, () => linha.tamanho);
      for (const tam of expandido) {
        const item = criarItem(pedido.id, {
          nome: linha.nome,
      ...
      // fotos originais da linha (frente, verso e patches) já não são precisas
      if (linha.foto) apagarFoto(linha.foto);
      if (linha.foto_verso) apagarFoto(linha.foto_verso);
