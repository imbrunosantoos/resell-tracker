...
const acertoParaApi = (r) => ({
  id: r.id, socioId: r.socio_id || null, valor: r.valor,
  sentido: r.sentido || "para_socio", data: r.data || "", nota: r.nota || "",
});
...
const CAMPOS_ACERTO = {
  valor: { col: "valor", conv: Number },
  sentido: { col: "sentido", conv: String },
  data: { col: "data", conv: String },
  nota: { col: "nota", conv: String },
};
...
export function lerEstado() {
  ...
  const acertos = db.prepare("SELECT * FROM acertos ORDER BY data DESC, criado_em DESC").all();
  ...
  return {
    ...
    acertos: acertos.map(acertoParaApi),
...
export function criarAcerto(dados = {}) {
  const id = randomUUID();
  getDB()
    .prepare("INSERT INTO acertos (id, socio_id, valor, sentido, data, nota, criado_em) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .run(
      id,
      dados.socioId ? String(dados.socioId) : null,
      num(dados.valor),
      dados.sentido === "do_socio" ? "do_socio" : "para_socio",
      String(dados.data || new Date().toISOString().slice(0, 10)),
      String(dados.nota ?? ""),
      new Date().toISOString()
    );
  return acertoParaApi(getDB().prepare("SELECT * FROM acertos WHERE id = ?").get(id));
}

export function apagarAcerto(id) {
  getDB().prepare("DELETE FROM acertos WHERE id = ?").run(id);
}
...
export function importarEstado(estado) {
  ...
  db.exec("BEGIN");
  try {
    db.exec("DELETE FROM acertos; DELETE FROM itens; DELETE FROM pedidos; DELETE FROM despesas; DELETE FROM socios;");
    ...
    for (const a of estado.acertos ?? []) criarAcerto(a); // sócios mantêm o id → socioId continua a bater
