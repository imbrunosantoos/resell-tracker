...
// Quanto já foi acertado (pago) a um sócio: soma dos acertos a favor dele
// (para_socio) menos o que recebeste dele (do_socio). O que falta pagar é a
// parte dele (lucroPorSocio) menos isto.
export function pagoAoSocio(acertos, socioId) {
  return (acertos ?? [])
    .filter((a) => a.socioId === socioId)
    .reduce((s, a) => s + (a.sentido === "do_socio" ? -toNumber(a.valor) : toNumber(a.valor)), 0);
}
