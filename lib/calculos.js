...
// Acerto de contas por venda: para cada sócio, metade do lucro de cada item
// vendido de um pedido dele é o que lhe tens de pagar. Marcar o item como
// "acertado" (item.acertado) tira-o do que falta. Devolve, por sócio:
//   devido    = Σ metade do lucro de todas as vendas dele
//   acertado  = Σ metade das que já marcaste como acertadas
//   falta     = devido − acertado
//   porAcertar = lista das vendas ainda por acertar (para a UI)
//   jaAcertadas = lista das já acertadas
export function acertoPorSocio(estado) {
  ...
  const por = new Map();
  for (const s of socios) por.set(s.id, { id: s.id, nome: s.nome, devido: 0, acertado: 0, falta: 0, porAcertar: [], jaAcertadas: [] });
  ...
  for (const pedido of pedidos) {
    ...
    for (const item of pedido.itens) {
      if (!estaVendido(item)) continue;
      const metade = margem(pedido, item) / 2;
      const venda = { pedido, item, metade };
      alvo.devido += metade;
      if (item.acertado) { alvo.acertado += metade; alvo.jaAcertadas.push(venda); }
...
// 'YYYY-MM' → "maio de 2026" (para títulos). Devolve o próprio valor se inválido.
export function nomeMes(mes) {
  const [ano, m] = String(mes || "").split("-").map(Number);
  if (!ano || !m) return mes || "";
  return new Date(ano, m - 1, 1).toLocaleDateString("pt-PT", { month: "long", year: "numeric" });
}
