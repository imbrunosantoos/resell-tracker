...
// A minha parte de uma despesa: inteira se for só minha, metade se for
// dividida com um sócio. As mensais multiplicam pelos meses ativos.
function minhaParteDespesa(d, meses) {
  const total = toNumber(d.valor) * (d.periodo === "mensal" ? meses : 1);
  return d.socioId ? total / 2 : total;
}

// A parte de UM sócio numa despesa: metade, e só se for dividida com ele.
function parteSocioDespesa(d, socioId, meses) {
  if (d.socioId !== socioId) return 0;
  return (toNumber(d.valor) * (d.periodo === "mensal" ? meses : 1)) / 2;
}
...
export function lucroPorSocio(estado) {
  ...
  for (const d of despesas) {
    minhasDespesas += minhaParteDespesa(d, meses);
    if (d.socioId && porSocio.has(d.socioId)) {
      porSocio.get(d.socioId).lucro -= parteSocioDespesa(d, d.socioId, meses);
    }
  }
...
export function dadosMensais(estado) {
  ...
  // a minha parte das despesas: por mês (mensais) e únicas (só no mês atual)
  const minhaMensal = despesas.reduce(
    (s, d) => (d.periodo === "mensal" ? s + (d.socioId ? toNumber(d.valor) / 2 : toNumber(d.valor)) : s), 0);
  const minhaUnica = despesas.reduce(
    (s, d) => (d.periodo !== "mensal" ? s + (d.socioId ? toNumber(d.valor) / 2 : toNumber(d.valor)) : s), 0);

  const mesAtual = chaveMes(new Date());
  const chaves = [...buckets.keys()].filter(Boolean).sort();
  ...
  return meses.map((mes) => {
    ...
    let meuLucro = b.meuVendas - minhaMensal;
    if (mes === mesAtual) meuLucro -= minhaUnica;
    return { mes, recebido: b.recebido, investido: b.investido, meuLucro, vendas: b.vendas };
  });
}
