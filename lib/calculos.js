...
export function resumoGlobal(estado) {
  ...
  for (const pedido of pedidos) {
    for (const item of pedido.itens) {
      ...
      // No total da categoria (sell-through) só entra o que já está na mão: itens
      // já vendidos ou de pedidos que já chegaram. Os que ainda estão a caminho
      // (pedido sem data de chegada) não contam — senão a % de vendido fica diluída.
      if (estaVendido(item) || pedido.dataChegada) categorias[cat].total++;
...
// O que está a ENTRAR (vendas pendentes, estimadas por uma data de recebimento
// = dataVenda + LAG_RECEBIMENTO) e o capital ainda PRESO: em trânsito (pedidos
// por chegar) e em stock (chegados, por vender). As datas são estimativa — na
// Vinted o dinheiro liberta alguns dias depois da venda. A minha parte conta
// metade nos pedidos com sócio (já vem assim de resumoVendas).
const LAG_RECEBIMENTO = 10; // dias típicos até o dinheiro cair (estimativa)

export function previsaoCaixa(estado, lagDias = LAG_RECEBIMENTO) {
  const { pendentes } = resumoVendas(estado);
  const hojeMs = Date.parse(new Date().toISOString().slice(0, 10));

  const baldes = { ate7: { valor: 0, n: 0 }, ate14: { valor: 0, n: 0 }, depois: { valor: 0, n: 0 } };
  let aReceber = 0;
  for (const p of pendentes) {
    aReceber += p.minhaParte;
    const venda = Date.parse((p.item.dataVenda || "").slice(0, 10));
    // numa encomenda o cliente paga na hora → recebido logo (lag 0); numa venda
    // normal (Vinted) o dinheiro liberta ~lagDias depois da venda.
    const lag = p.pedido.tipo === "encomenda" ? 0 : lagDias;
    // dias até ao recebimento estimado (negativo = já devia ter caído → iminente)
    const faltam = Number.isNaN(venda)
      ? 0
      : Math.round((venda + lag * 86_400_000 - hojeMs) / 86_400_000);
    const balde = faltam <= 7 ? "ate7" : faltam <= 14 ? "ate14" : "depois";
    baldes[balde].valor += p.minhaParte;
    baldes[balde].n++;
  }

  let emTransito = 0, emStock = 0;
  for (const pedido of estado.pedidos ?? []) {
    for (const item of pedido.itens) {
      if (estaVendido(item)) continue;
      const custo = custoReal(pedido, item);
      if (pedido.dataChegada) emStock += custo; else emTransito += custo;
    }
  }

  return {
    aReceber, emTransito, emStock,
    baldes: [
      { chave: "ate7", ...baldes.ate7 },
      { chave: "ate14", ...baldes.ate14 },
      { chave: "depois", ...baldes.depois },
    ],
  };
}

// ---------- Caixa disponível ----------
// Dinheiro que já VOLTOU (a minha parte das vendas recebidas) menos o que
// SAIU (a minha parte do custo de todos os itens + as minhas despesas até
// agora). Positivo = cash líquido em mãos; negativo = capital por recuperar.
// Recuperar capital ≠ lucrar: este número é caixa, não margem.
export function caixaDisponivel(estado) {
  const pedidos = estado.pedidos ?? [];
  const socios = estado.socios ?? [];
  const despesasLista = estado.despesas ?? [];
  const idsValidos = new Set(socios.map((s) => s.id));
  const mesAtual = chaveMes(new Date());

  let recebido = 0, investido = 0;
  for (const pedido of pedidos) {
    const comSocio = pedido.socioId && idsValidos.has(pedido.socioId);
    const parte = (v) => (comSocio ? v / 2 : v);
    for (const item of pedido.itens) {
      investido += parte(custoReal(pedido, item));
      if (estaRecebido(item)) recebido += parte(toNumber(item.precoVenda));
    }
  }
  const despesas = despesasLista.reduce((s, d) => s + minhaParteBase(d) * ocorrencias(d, mesAtual), 0);
  return { recebido, investido, despesas, saldo: recebido - investido - despesas };
}
...
export function relatorioMensal(estado) {
  ...
  // A minha parte do lucro das vendas do mês `mes` até ao dia `ateDia`
  // (inclusive; null = mês inteiro), menos a minha parte das despesas desse
  // mês. A despesa mensal pesa por inteiro nos DOIS períodos — enviesamento
  // simétrico, comparação justa de "mesmo período".
  function lucroAte(mes, ateDia) {
    let vendasN = 0, lucro = 0;
    for (const pedido of pedidos) {
      const comSocio = pedido.socioId && idsValidos.has(pedido.socioId);
      for (const item of pedido.itens) {
        if (!estaVendido(item)) continue;
        if (item.dataVenda.slice(0, 7) !== mes) continue;
        if (ateDia !== null && Number(item.dataVenda.slice(8, 10)) > ateDia) continue;
        const m = margem(pedido, item);
        lucro += comSocio ? m / 2 : m;
        vendasN++;
      }
    }
    const despesaMes = despesas.reduce((s, d) => s + minhaParteBase(d) * contaNoMes(d, mes), 0);
    return { lucro: lucro - despesaMes, vendas: vendasN };
  }

  const atual = lucroAte(kAtual, dia);
  const anteriorMesmoPeriodo = lucroAte(kAnt, dia);
  const anteriorCompleto = lucroAte(kAnt, null);

  // A variação compara MESMO período (este mês até dia N vs anterior até dia N)
  // — comparar acumulado parcial com mês fechado assusta à toa.
  return {
    dia,
    atual: { ...atual, label: kAtual },
    anterior: { ...anteriorCompleto, label: kAnt },
    anteriorMesmoPeriodo,
    variacao: atual.lucro - anteriorMesmoPeriodo.lucro,
  };
}
...
// Lucro por MODELO (pelo nome do item, ex.: "Real Madrid Home 24/25"). Mesma
// forma que as categorias — lucro, sell-through e dias médios — mas agrupado
// pelo nome que já escreves, sem catálogo a manter. Ordenado por lucro.
export function lucroPorModelo(estado) {
  const pedidos = estado.pedidos ?? [];
  const map = {}; // nome -> { lucro, vendidos, total, somaDias, comDias }
  for (const pedido of pedidos) {
    // só pedidos normais (aba Pedidos) — as encomendas não entram nesta análise
    if ((pedido.tipo || "normal") === "encomenda") continue;
    for (const item of pedido.itens) {
      const nome = item.nome?.trim() || "Sem nome";
      map[nome] ??= { lucro: 0, vendidos: 0, total: 0, somaDias: 0, comDias: 0 };
      // total "na mão": vendido ou de pedido já chegado (igual às categorias)
      if (estaVendido(item) || pedido.dataChegada) map[nome].total++;
      if (estaVendido(item)) {
        map[nome].lucro += margem(pedido, item);
        map[nome].vendidos++;
        const d = diasParaVender(pedido, item);
        if (d !== null) { map[nome].somaDias += d; map[nome].comDias++; }
      }
    }
  }
